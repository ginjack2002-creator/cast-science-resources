...
Your model shows a relationship between Cell Division Rate and Mutation Frequency,
but right now that relationship applies ALL the time. In the
real system, Cell Division Rate only affects Mutation Frequency when UV Radiation is active.
...
Task A: SET THE CONDITION
   • Click on the connection arrow from Cell Division Rate to Mutation Frequency
   • In the toolbar, click "Conditions"
   • In the conditions panel, set: IF UV Radiation is ON
   • Click "Save Conditions"

Task B: VERIFY THE MODEL
   • Run the simulation with UV Radiation OFF and Cell Division Rate ON
   • Mutation Frequency should NOT respond to Cell Division Rate alone
   • Now turn UV Radiation ON — Cell Division Rate's effect on Mutation Frequency activates
...
B. Lethal — the organism will die eventually from accumulated damage
C. Beneficial — the protein must have improved
D. Neutral — the change didn't significantly affect protein function
...
A. Skin cells accumulate more mutations because each division is another chance for copying errors.
B. Both accumulate mutations at the same rate regardless of division.
C. Nerve cells accumulate more mutations because they are older.
...
B. Mutation Frequency is unpredictable without cell division data.
C. Mutation Frequency increases, then decreases as the cell repairs itself.
D. Mutation Frequency stays low because mutations only occur during DNA copying, and no copying is happening.
...
A. Evolution doesn't actually require mutations — it works through environmental pressure alone.
B. Harmful mutations strengthen species by killing off weak individuals.
C. Across large populations and many generations, the rare beneficial mutation provides new traits for natural selection to act on.
D. Mutations are actually mostly beneficial, but scientists haven't discovered the benefits yet.
