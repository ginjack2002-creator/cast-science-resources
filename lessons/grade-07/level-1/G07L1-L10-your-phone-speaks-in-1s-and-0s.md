...
Your model shows a relationship between Sampling Rate and Signal Accuracy,
but right now that relationship applies ALL the time. In the
real system, Sampling Rate only affects Signal Accuracy when Compression Level is inactive.
...
Task A: SET THE CONDITION
   • Click on the connection arrow from Sampling Rate to Signal Accuracy
   • In the toolbar, click "Conditions"
   • In the conditions panel, set: IF Compression Level is OFF
   • Click "Save Conditions"

Task B: VERIFY THE MODEL
   • Run the simulation with Compression Level OFF and Sampling Rate ON
   • Signal Accuracy should NOT respond to Sampling Rate alone
   • Now turn Compression Level ON — Sampling Rate's effect on Signal Accuracy activates
   • This matches how the real system behaves

Task C: SET ADDITIONAL CONDITION
   • Click on the arrow from Compression Level to Signal Accuracy
   • Click "Conditions" and set: IF Sampling Rate is ON
   • Save — Compression Level now only affects Signal Accuracy when
     Sampling Rate is active
...
CAST Alignment: SEP 2 (Developing and Using Models) + DCI PS4.C (Digitized signals and information technologies) + CCC3 (Scale, Proportion, and Quantity)

In a digital signal model, a student adjusts two sliders: sampling rate (how many times per second the original analog sound wave is measured) and compression level (how much data is removed to reduce file size). She sets sampling rate to maximum and compression to minimum and runs the model. The output shows a digital waveform that almost perfectly matches the original analog sound wave. The file size indicator shows 50 megabytes for a 3-minute song.

What does the model predict about the relationship between signal accuracy and file size at these settings?

A. Both signal accuracy and file size are at minimum values
B. Signal accuracy and file size are completely unrelated to sampling rate and compression settings
C. Signal accuracy is very high and file size is very large because maximum sampling captures the most detail, and minimum compression preserves all of it
D. Signal accuracy is low but file size is very small because maximum sampling removes detail

Correct Answer: C

Feedback: Maximum sampling rate measures the original signal as frequently as possible, capturing fine details. Minimum compression keeps all that data intact. The result is near-perfect accuracy but very large files. This is the quality setting used in professional recording studios, where accuracy matters more than storage space. If you chose D, maximum sampling captures more detail, not less. Higher sampling rates produce more accurate digital representations. If you chose A, the settings described (max sampling, min compression) produce maximum accuracy and maximum file size, not minimum values. If you chose B, the model directly demonstrates that these settings control both accuracy and file size. Sampling rate determines how much detail is captured, and compression determines how much is kept.
---

### Question 2

CAST Alignment: SEP 7 (Engaging in Argument from Evidence) + DCI PS4.C (Analog vs. digital signal quality) + CCC6 (Structure and Function)

A student claims: 'Digital music is always better quality than analog vinyl records.' Her friend, an audiophile, disagrees and plays a heavily compressed 64-kbps MP3 file next to a well-maintained vinyl record of the same song. The class agrees that the vinyl sounds noticeably richer and more detailed. The teacher then plays an uncompressed 1,411-kbps CD-quality digital file of the same song, and the class agrees it sounds as good as or better than the vinyl.

Based on the listening comparison, which conclusion about digital vs. analog audio is most scientifically accurate?

A. Digital audio is more reliable because it can be copied perfectly without degradation, but the perceived quality depends on sampling rate and compression settings. Heavily compressed digital can sound worse than well-maintained analog.
B. The first student is correct. Digital audio is always superior to analog in every situation.
C. Analog vinyl is always superior because it captures continuous sound waves without any loss whatsoever
D. Digital and analog produce identical sound quality because they use the same technology

Correct Answer: A

Feedback: The key advantage of digital is reliability: perfect copies with zero degradation. However, the quality of digital audio depends entirely on sampling rate and compression settings. A 64-kbps MP3 has lost so much data that it sounds worse than vinyl. A 1,411-kbps uncompressed file captures enough detail to match or exceed vinyl. If you chose B, the listening test demonstrated that heavily compressed digital can sound worse than vinyl, disproving the 'always better' claim. If you chose C, vinyl is not perfect. Records degrade with each play, collect dust, and can warp. They also have inherent limitations like surface noise and frequency range. If you chose D, analog and digital use fundamentally different technologies. Analog stores continuous physical patterns (grooves), while digital stores discrete numerical values (binary).
---

### Question 3

CAST Alignment: SEP 4 (Analyzing and Interpreting Data) + DCI PS4.C (Effects of compression on signal quality) + CCC2 (Cause and Effect)

A student uses the digital signal model to test what happens at extreme compression levels. She starts with an uncompressed audio file that sounds identical to the original recording. She then applies increasing levels of compression: at 256 kbps, the sound is indistinguishable from the original to most listeners. At 128 kbps, trained musicians notice slight differences in high frequencies. At 64 kbps, everyone can hear that the music sounds thin and metallic. At 32 kbps, the music is barely recognizable, with severe artifacts and distortion.

Based on the model's data, what happens to audio quality when compression level is pushed to maximum?

A. Audio quality stays the same because compression only reduces file size without affecting the listening experience
B. Audio quality drops progressively as more data is removed. At extreme compression, the algorithm removes enough audio information that humans can clearly hear the degradation as thin, metallic, or distorted sound.
C. The music plays faster because there is less data for the player to process
D. Compression makes the music louder by squeezing sound waves closer together

Correct Answer: B

Feedback: Compression algorithms work by removing audio data that they predict humans will not notice. At moderate levels, this works well. But at extreme levels, the algorithm must remove data that IS noticeable, causing audible artifacts. The sound becomes thin because harmonic detail is stripped, metallic because natural resonance is lost, and distorted because the algorithm can no longer accurately represent the original waveform. If you chose A, the student's data clearly shows that quality degrades at each compression level. At 32 kbps, the music is 'barely recognizable,' proving compression absolutely affects quality. If you chose C, compression does not change playback speed. It reduces the amount of data stored per second of audio, which affects quality, not speed. If you chose D, audio compression is not physical compression. It reduces file size by removing data points, not by physically pressing sound waves together.
---

### Question 4

CAST Alignment: SEP 8 (Obtaining, Evaluating, and Communicating Information) + DCI PS4.C (Reliability of digital vs. analog transmission) + CCC6 (Structure and Function)

A communications engineer explains why digital signals replaced analog for most information transmission. She shows two demonstrations. In Demo 1, she makes an analog photocopy of a document, then photocopies the copy, then photocopies that copy 10 times. By the 10th copy, the text is blurry and barely readable. In Demo 2, she copies a digital text file 10 times. The 10th copy is absolutely identical to the original, with every letter, space, and punctuation mark exactly preserved.

Why are digital signals considered fundamentally more reliable than analog signals for storing and transmitting information?

A. Digital signals encode information as exact numerical values (binary 1s and 0s) that can be copied perfectly every time, while analog signals are physical patterns that degrade with each copy or transmission
B. Digital equipment is physically stronger and more durable than analog equipment
C. Digital signals use more electricity, which makes them more powerful and accurate
D. Analog signals can only travel short distances, while digital signals can travel worldwide

Correct Answer: A

Feedback: The fundamental advantage of digital is that information is stored as numbers, and numbers can be copied exactly. The value 01100001 is always 01100001, no matter how many times it is copied. Analog signals are physical patterns (grooves, magnetic alignments, voltage fluctuations) that inevitably degrade when copied or transmitted because each reproduction introduces small errors that accumulate. If you chose B, the advantage is in the signal format, not the equipment. A perfectly maintained analog tape still degrades with copies, while a scratched digital disc can still produce perfect data through error correction. If you chose D, both analog and digital signals can travel long distances. Radio (analog) spans the globe, and fiber optic (digital) spans oceans. The advantage is copy fidelity, not range. If you chose C, power consumption is unrelated to signal reliability. Digital's advantage comes from encoding information as discrete values that can be verified and error-corrected.
---

### Question 5

CAST Alignment: SEP 6 (Constructing Explanations) + DCI PS4.C (Trade-offs in signal design) + CCC3 (Scale, Proportion, and Quantity)

A streaming music service offers three quality tiers: Low (96 kbps, uses 43 MB per hour), Standard (160 kbps, uses 72 MB per hour), and High (320 kbps, uses 144 MB per hour). A student on a road trip has a phone plan with only 2 GB of mobile data remaining for the month. She wants to listen to as much music as possible during her 10-hour drive but also wants good sound quality. She uses the digital signal model to understand the trade-offs.

Based on the model's quality-vs-size trade-off, what is the best strategy for the student?

A. Use Low quality for the entire trip because data conservation is the only factor that matters, regardless of sound quality
B. The quality setting has no effect on data usage, so she should always choose High
C. Use High quality because sound quality is the only factor that matters, regardless of data usage
D. Use Standard quality, which provides a balance between acceptable sound quality and reasonable data usage, allowing approximately 28 hours of listening within her 2 GB limit

Correct Answer: D

Feedback: This is the quality-vs-size trade-off the model demonstrates. Standard quality at 72 MB/hour allows about 28 hours of listening within 2 GB, more than covering the 10-hour trip with data to spare. High quality would use 144 MB/hour, lasting only 14 hours but consuming her entire data budget. Low quality would last 46 hours but with noticeably degraded sound. Standard offers the best balance. If you chose C, at 144 MB/hour, High quality would consume 1.44 GB of her 2 GB in just 10 hours, leaving almost nothing for the rest of the month. Ignoring data constraints is not practical. If you chose A, while Low quality maximizes listening time, the question asks for a balance between quality and data usage. Low quality has noticeable audio degradation that most listeners would find unpleasant over 10 hours. If you chose B, the data in the question clearly shows that quality settings directly affect data usage: Low uses 43 MB/hour while High uses 144 MB/hour, a 3.3 times difference.
---

### Answer Key

Question 1: C (Cognitive Level: Identify — SEP 2 (Developing and Using Models), DCI PS4.C, CCC3 (Scale, Proportion, and Quantity))
Question 2: A (Cognitive Level: Reason — SEP 7 (Engaging in Argument from Evidence), DCI PS4.C, CCC6 (Structure and Function))
Question 3: B (Cognitive Level: Reason + Evidence — SEP 4 (Analyzing and Interpreting Data), DCI PS4.C, CCC2 (Cause and Effect))
Question 4: A (Cognitive Level: Reason — SEP 8 (Obtaining, Evaluating, and Communicating Information), DCI PS4.C, CCC6 (Structure and Function))
Question 5: D (Cognitive Level: Predict + Apply — SEP 6 (Constructing Explanations), DCI PS4.C, CCC3 (Scale, Proportion, and Quantity))
...
| Resource | Link |
|----------|------|
| Activity Pack (DOCX) | [materials/grade-07/level-1/G07L1-L10/G07L1-L10-Student-Activity-Pack.docx] |
| Teacher Guide (DOCX) | [materials/grade-07/level-1/G07L1-L10/G07L1-L10-Teachers-Guide.docx] |
| PPT Presentation | [materials/grade-07/level-1/G07L1-L10/G07L1-L10-Student-Presentation.pptx] |
