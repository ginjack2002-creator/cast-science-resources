#!/usr/bin/env python3
"""ModelIt! Batch Generator - Nature's Engineers Series (NE-L1-01 through NE-L2-06)"""
import os, sys, time, base64, requests
from lesson_data_natures_engineers import ALL_LESSONS
...
BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'materials', 'natures-engineers')
...
# Grade map: lesson ID prefix -> (grade label, age range)
GRADE_MAP = {
    "NE-L1-01": ("Grades 3-5", "8-11 years old"),
    "NE-L2-01": ("Grades 3-5", "8-11 years old"),
    "NE-L1-02": ("Grades 4-8", "9-14 years old"),
    "NE-L2-02": ("Grades 4-8", "9-14 years old"),
    "NE-L1-03": ("Grades 6-8", "11-14 years old"),
    "NE-L2-03": ("Grades 6-8", "11-14 years old"),
    "NE-L1-04": ("Grades 6-10", "11-16 years old"),
    "NE-L2-04": ("Grades 6-10", "11-16 years old"),
    "NE-L1-05": ("Grades 8-12", "13-18 years old"),
    "NE-L2-05": ("Grades 8-12", "13-18 years old"),
    "NE-L1-06": ("Grades 9-12", "14-18 years old"),
    "NE-L2-06": ("Grades 9-12", "14-18 years old"),
}

def get_grade_info(lid):
    """Return (grade_label, age_range) for a given lesson ID."""
    if lid in GRADE_MAP:
        return GRADE_MAP[lid]
    return ("Grades 3-12", "8-18 years old")

def gen_img(scene, fn, img_dir, age_range):
    global total_cost
    prompt = (f"Create a photorealistic, cinematic, ultra-crisp image of {scene} "
              "featuring a genuinely diverse group reflecting US public school diversity -- Latin American, Black, Asian/AAPI, and White students all represented, with varied facial features and hairstyles (straight, wavy, curly, coily, braids, locs). Everyone in everyday modern clothing (no cultural or traditional outfits). Rotate who takes the lead "
              "(a mix of skin tones and ethnicities represented), age-accurate for "
              f"{age_range} (no one looks like an adult if they're a kid). "
              "Style: modern education / future-ready / elementary school confidence, "
              "confident, aspirational, realistic. Composition: clean framing, "
              "natural body proportions, realistic hair detail (straight, wavy, curly, coily, braids, locs), varied facial features, no twins or clone faces. "
...
def gen_all_imgs(c, img_dir, age_range):
    ...
    for key,(fn,scene) in c["images"].items():
        ...
        if os.path.exists(fp):
            ...
            imgs[key]=gen_img(scene,fn,img_dir,age_range); time.sleep(3)
    return imgs
...
def make_guide(c, out_dir, grade_label):
    ...
    qr=[[" Grade Level",grade_label],
        ...
        ["Time Required","35-45 minutes (can extend to 60 min with STEM challenge)"],
    ...
    # Dynamic row count: header row + max(len(ext), len(int)) data rows
    max_rows = max(len(ext_names), len(int_names))
    if max_rows < 1:
        max_rows = 1
    sort_t=doc.add_table(rows=1+max_rows,cols=2);sort_t.style="Table Grid"
    ...
    for ri in range(max_rows):
        sort_t.rows[1+ri].cells[0].text = ext_names[ri] if ri < len(ext_names) else ""
        sort_t.rows[1+ri].cells[1].text = int_names[ri] if ri < len(int_names) else ""
        for cell in sort_t.rows[1+ri].cells:
    ...
    for entry in c["stem_eval"]:
        if len(entry) == 2:
            lv2, criteria = entry
        elif len(entry) == 3:
            lv2, criteria, _ = entry
        else:
            lv2 = str(entry[0]) if entry else ""
            criteria = str(entry[1]) if len(entry) > 1 else ""
    ...
    doc.add_page_break()
    # ========== SECTION 9: GAME & STEAM ACTIVITY ==========
    hdr(doc,"9. Game & STEAM Activity")
    # Game section
    game = c.get("game", {})
    if game:
        hdr(doc, f"Game: {game.get('title', 'N/A')}", lv=2)
        p=doc.add_paragraph();r=p.add_run("Type: ");r.bold=True;r.font.size=DP(11)
        p.add_run(game.get("type", "")).font.size=DP(11)
        doc.add_paragraph()
        p=doc.add_paragraph();r=p.add_run("Description: ");r.bold=True;r.font.size=DP(11)
        p.add_run(game.get("description", "")).font.size=DP(11)
        doc.add_paragraph()
        materials_list = game.get("materials", [])
        if materials_list:
            p=doc.add_paragraph();r=p.add_run("Materials Needed:");r.bold=True;r.font.size=DP(11)
            for mat in materials_list:
                p=doc.add_paragraph(style="List Bullet");p.add_run(mat).font.size=DP(11)
        doc.add_paragraph()
        p=doc.add_paragraph();r=p.add_run("Learning Connection: ");r.bold=True;r.font.size=DP(11);r.font.color.rgb=DMB
        p.add_run(game.get("learning_connection", "")).font.size=DP(11)
        doc.add_paragraph()
    # STEAM Activity section
    steam = c.get("steam_activity", {})
    if steam:
        hdr(doc, f"STEAM Activity: {steam.get('title', 'N/A')}", lv=2)
        p=doc.add_paragraph();r=p.add_run("Type: ");r.bold=True;r.font.size=DP(11)
        p.add_run(steam.get("type", "")).font.size=DP(11)
        doc.add_paragraph()
        p=doc.add_paragraph();r=p.add_run("Description: ");r.bold=True;r.font.size=DP(11)
        p.add_run(steam.get("description", "")).font.size=DP(11)
        doc.add_paragraph()
        materials_list = steam.get("materials", [])
        if materials_list:
            p=doc.add_paragraph();r=p.add_run("Materials Needed:");r.bold=True;r.font.size=DP(11)
            for mat in materials_list:
                p=doc.add_paragraph(style="List Bullet");p.add_run(mat).font.size=DP(11)
...
if __name__ == "__main__":
    print("="*60)
    print("ModelIt! Batch Generator - Nature's Engineers Series")
    print("NE-L1-01 through NE-L2-06 (Grades 3-12)")
    print("="*60)
    print(f"\nGenerating {len(ALL_LESSONS)} Nature's Engineers lessons:")
    for lesson in ALL_LESSONS:
        gl, _ = get_grade_info(lesson["id"])
        print(f"  - {lesson['id']}: {lesson['title']} ({gl})")
    print()
